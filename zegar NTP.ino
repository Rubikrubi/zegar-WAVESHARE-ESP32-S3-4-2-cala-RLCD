// ============================================================================
//  Zegar NTP/RTC — wersja polaczona (ekran/klawisze/WWW z "ZegarNTP3 Ultra v.1.3
//  korekta" + mechanizmy oszczedzania energii z "ZegarNTP_LowPower").
//  Cel: maksymalny czas pracy na ogniwie Li-ion 3400 mAh, BEZ utraty:
//    - dokladnosci czasu w ciagu dnia (do 2 s) — light-sleep do granicy minuty
//      z re-kotwiczeniem zegara z RTC po kazdym wybudzeniu,
//    - braku mrugania ekranu (light-sleep zachowuje RAM, RLCD trzyma obraz).
//  Ekran, obsluga przyciskow i panel WWW = identyczne jak w v1.3 (sprawdzone).
//  Energia: nieliniowa krzywa OCV->SoC, FAST_NTP_RECONNECT, POWER_DOWN_FLASH
//  w light-sleep, idle-gating pomiaru baterii, estymator zuzycia mAh/dobe.
//
//  Deliverable: JEDEN plik .ino (dawny power_runtime.h wtopiony nizej) + osobny
//  photo_font.h. Komunikaty po polsku (font bez polskich znakow -> ASCII).
//
...
static bool writeRtc(time_t epoch) {
  ...
  bool clockReleased = rtcReleaseClock(runningControl1);
  if (!registersWritten || !clockReleased) return false;
  ...
  long long rtcDifference = (long long)verifiedEpoch - (long long)epoch;
...
void setup() {
  ...
  // Zimny start zawsze koryguje RTC z NTP. Po wybudzeniu z deep-sleep

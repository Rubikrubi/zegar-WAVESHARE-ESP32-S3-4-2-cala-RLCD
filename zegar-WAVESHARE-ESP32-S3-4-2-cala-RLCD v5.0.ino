...
//  Zegar NTP v5.0 — 24.06.2026 21:41 - finalna wersja produkcyjna
...
//
//  Hardening v5.0 (przelaczniki ENABLE_* ponizej): watchdog zadania z obsluga
//  snu, ochrona niskiego stanu ogniwa (degradacja + ochronny sen), diagnostyka
//  /info (przyczyna resetu, czas pracy, heap, dryft RTC), wersjonowanie + build
//  info. WiFi/NTP, OTA, factory-reset i logowanie panelu WWW celowo bez zmian.
// ============================================================================
...
#include "esp_sntp.h"
#include "esp_task_wdt.h"   // watchdog zadania (TWDT) — hardening 
#include "esp_system.h"     // esp_reset_reason() — diagnostyka 

#include "photo_font.h"

// Deklaracje wyprzedzajace struktur uzywanych w sygnaturach funkcji. MUSZA stac
// przed PIERWSZA definicja funkcji w pliku (watchdogSetup ponizej): Arduino
// auto-generuje prototypy WSZYSTKICH funkcji i wstawia je tuz przed pierwsza
// definicje, w tym drain*(DrainState&) i sanitizeAlarm(AlarmConfig&). Bez nich
// w tym miejscu kompilator zglasza "'DrainState' was not declared". NIE usuwac.
struct AlarmConfig;
struct DrainState;

// ============================================================================
//  WERSJA I INFORMACJE O KOMPILACJI (build info)
// ============================================================================
#define FW_NAME        "Zegar NTP RLCD"
#define FW_VERSION     "5.0"          // finalna wersja produkcyjna
#define FW_BUILD_DATE  __DATE__       // data kompilacji (np. "Jun 22 2026")
#define FW_BUILD_TIME  __TIME__       // godzina kompilacji (np. "18:55:01")

// ============================================================================
//  FLAGI FUNKCJI PRODUKCYJNYCH (hardening) — 1=wlaczone, 0=wylaczone
//  Centralny przelacznik elementow "finalnych". Ustawienie flagi na 0 usuwa jej
//  kod z kompilacji (#if) i przywraca zachowanie sprzed v4.0 — zero narzutu.
// ============================================================================
...
#define ENABLE_DIAGNOSTICS         1   // strona /info, liczniki resetow, dryft RTC

// ---------------------------------------------------------------------------
//  Watchdog zadania (TWDT)
//  Timeout MUSI byc wiekszy niz najdluzszy light-sleep dzienny
//  (IDLE_LIGHT_SLEEP_MAX_SEC = 60 s). W light-sleep licznik TWDT zwykle stoi
//  (zegar APB bramkowany), ale margines ponad 60 s gwarantuje brak falszywego
//  resetu NIEZALEZNIE od wersji rdzenia ESP32. Realne zawieszenie petli i tak
//  zostaje wylapane w < WDT_TIMEOUT_S. Dlugie operacje blokujace (upload MP3,
//  laczenie WiFi) jawnie karmia watchdog, wiec nie wywoluja resetu.
//  Nocny deep-sleep i ochronny sen rozladowania RESETUJA chip -> TWDT startuje
//  od nowa w setup(), wiec nie wymagaja osobnej obslugi w czasie snu.
// ---------------------------------------------------------------------------
constexpr uint32_t WDT_TIMEOUT_S = 90;

// Inicjalizacja watchdoga dla petli setup()/loop(). Rdzen Arduino zwykle juz
// zainicjowal TWDT (nadzor IDLE) — wtedy tylko rekonfigurujemy timeout. Na
// starszych rdzeniach (2.x) esp_task_wdt_init() ma inna sygnature.
static inline void watchdogSetup() {
  ...
  #endif
  esp_task_wdt_add(NULL);          // nadzoruj biezacy task (loopTask: setup+loop)
  esp_task_wdt_reset();
#endif
}

// Nakarm watchdoga. Wolane na poczatku loop() oraz przed/po snach i dlugich
// operacjach blokujacych (upload MP3, laczenie WiFi).
...
// ---------------------------------------------------------------------------
//  Detektor brownout (spadku napiecia)
//  ESP32-S3 ma sprzetowy brownout-detector wlaczany przez rdzen Arduino
//  (sdkconfig CONFIG_ESP_BROWNOUT_DET, prog domyslny ~2.51 V). CELOWO zostaje
//  WLACZONY: przy zalamaniu napiecia natychmiast resetuje uklad, co chroni FLASH
//  przed uszkodzeniem w trakcie zapisu. Progu nie zmieniamy — w jednoplikowym
//  szkicu .ino nie ma do tego API; wymaga to przebudowy sdkconfig. Zamiast tego
//  programowa ochrona ogniwa (ENABLE_LOW_BATTERY_PROTECT) usypia uklad juz przy
//  ~3.05 V, czyli DUZO powyzej progu sprzetowego — w normalnej pracy brownout nie
//  powinien wystapic. Kazdy reset brownout jest zliczany i widoczny na /info;
//  niezerowy licznik = sygnal slabnacego ogniwa lub przeciazenia.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Powtorzenie deklaracji wyprzedzajacych z poczatku pliku (blok przy watchdogSetup).
// W obecnym ukladzie struktury sa juz zadeklarowane wyzej, wiec tu sa nadmiarowe;
// trzymane lokalnie przy bloku power_runtime, by pozostal samodzielny przy
// ewentualnym wydzieleniu do osobnego pliku. NIE usuwac bez przeniesienia deklaracji.
struct AlarmConfig;
struct DrainState;
...
// Zmierzony dryft: -2 s / 24 h => RTC spoznia sie o ~23,1 ppm (krysztal za wolny).
// MODE=0 (korekcja co 2 h): 1 LSB = 4,34 ppm; wartosc DODATNIA PRZYSPIESZA zegar.
// 23,1 / 4,34 = 5,3 -> +5 LSB = +21,7 ppm; reszta dryftu ~0,13 s / 24 h (<< 1 s).
// Bit 7 (MODE) = 0, bity 6:0 = +5 (uzupelnienie do dwoch) -> bajt 0x05.
#define RTC_OFFSET_REG       0x02
#define RTC_OFFSET_VALUE     0x05
...
// --- Ochrona niskiego stanu akumulatora (low-battery protection) ------------
//  Dwustopniowa. LOW: degradacja — wstrzymaj auto-NTP/WiFi (najwiekszy pobor
//  pradu); zegar dalej chodzi z RTC, budziki dzialaja, reczny sync z WWW dziala.
//  CRITICAL: ochronny deep-sleep, by NIE rozladowac ogniwa Li-ion ponizej
//  bezpiecznego progu i nie wywolac sprzetowego brownoutu (ryzyko uszkodzenia
//  FLASH przy zapisie). Progi powyzej BATTERY_EMPTY_VOLTAGE (3.00 V) i znacznie
//  powyzej brownoutu sprzetowego, mierzone w spoczynku (OCV).
constexpr uint8_t  BATTERY_LOW_PERCENT          = 15;     // <= 15% -> tryb oszczedny (bez WiFi/NTP)
constexpr float    BATTERY_CRITICAL_VOLTAGE     = 3.05f;  // <= 3.05 V -> ochronny sen
constexpr uint8_t  BATTERY_CRITICAL_PERCENT     = 3;      //   lub <= 3%
constexpr uint32_t BATTERY_CRITICAL_RECHECK_SEC = 30UL * 60UL;  // co ile budzic sie sprawdzic ladowanie
...
constexpr uint32_t WIFI_HINT_FALLBACK_MS      = 5000;   // po nieudanym szybkim BSSID przejdz na pelny skan AP
...
constexpr uint32_t NIGHT_FALLBACK_ALARM_CHUNK_SEC = 300;  // jw., ale gdy najblizszym zdarzeniem jest budzik: krotsze odcinki -> dryft RC ostatniego << NTP_ALARM_CATCHUP_SEC (90 s), inaczej spoznione wybudzenie gubi budzik
...
//  Obiekty globalne
// ---------------------------------------------------------------------------
// RST = U8X8_PIN_NONE: U8g2 nigdy sam nie pulsuje resetu. RST sterujemy recznie w
// setupDisplay() przy KAZDYM wejsciu w setup() (zimny start ORAZ wybudzenie z
// deep-sleep — ten egzemplarz ST7305 nie trzyma obrazu przez deep-sleep, patrz
// setupDisplay). W dzien petla uzywa light-sleep, wiec setup()/setupDisplay() NIE
// wykonuja sie ponownie: panel nie jest resetowany i trzyma obraz -> brak mrugania.
...
// RTC RAM: przezywa deep-sleep (noc / krytyczna bateria rebootuja uklad). Bez tego
// kazde wybudzenie z deep-sleep zerowaloby je i histereza "trzymaj 100% gdy pomiar
// >= 98%" (stabilizePercent) by nie dzialala -> ekran migalby 99<->100 przy pelnym
// ogniwie. Zerowane na cold boot.
...
// Diagnostyka produkcyjna. RTC RAM przezywa deep-sleep (noce/budziki), znika po
// zaniku zasilania = jeden cykl pracy ogniwa. Zerowane na cold boot w setup().
static RTC_DATA_ATTR uint16_t g_bootCount;        // liczba rozruchow od zimnego startu
static RTC_DATA_ATTR int64_t  g_coldBootEpoch;    // czas pierwszego poprawnego odczytu (UTC, 0=jeszcze nieznany)
static RTC_DATA_ATTR int32_t  g_lastRtcDriftSec;  // RTC - NTP przy ostatniej synchronizacji
static RTC_DATA_ATTR int64_t  g_lastNtpSyncEpoch; // moment ostatniej udanej synchronizacji NTP (UTC)
static RTC_DATA_ATTR int64_t  g_prevNtpSyncEpoch; // poprzednia synchronizacja (okno do liczenia dryftu)
// Dozywotnie liczniki "zlych" resetow — backowane w NVS (zdarzenia rzadkie).
...
// Sygnatura widocznej tresci strony zegara (patrz clockSig). Sentinel 0xFFFFFFFF
// wymusza repaint (cold boot, powrot z innej strony). Dzieki niej zmiany pol
// niepokazywanych na zegarze (ntpStatus/statusText) NIE odswiezaja panelu.
static uint32_t lastClockSig = 0xFFFFFFFFu;
...
// RTC RAM: przezywa deep-sleep (kazde wybudzenie z deep-sleep rebootuje uklad i bez
// RTC RAM zerowaloby znacznik -> ponowna synchronizacja NTP po wybudzeniu, mimo ze
// dzis juz zsynchronizowano). Zerowany na cold boot.
...
// Pojedynczy zapis rejestru RTC. Wolajacy odpowiada za wczesniejsze rtcBeginSession()
// (gdy jest wymagane) — tak samo jak dotychczasowe rozwiniete idiomy.
...
// Odczyt n rejestrow RTC od 'reg' jednym transferem. NIE wola rtcBeginSession():
// wolajacy juz otworzyl sesje. Zachowanie identyczne z dotychczasowymi idiomami
// (te same casty, ta sama kolejnosc, ten sam warunek bledu).
...
static bool rtcWriteControl1(uint8_t control1) {
  return rtcWriteReg(0x00, control1);
}
...
static bool rtcHasCurrentFormat() {
  uint8_t marker;                  // 0x03 RAM_byte — bez rtcBeginSession (sesja juz otwarta)
  return rtcReadRegs(0x03, &marker, 1) && marker == RTC_FORMAT_MARKER;
}

static bool rtcStoreCurrentFormat() {
  return rtcWriteReg(0x03, RTC_FORMAT_MARKER);   // RAM_byte
}

static bool rtcInvalidateCurrentFormat() {
  return rtcWriteReg(0x03, 0);                    // RAM_byte
}
...
static bool writeRtc(time_t epoch) {
  ...
  // Pomin wyrownanie gdy gra audio/budzik lub trwa sesja WWW: busy-wait do ~1 s
  // przerwalby audio.loop() (underrun I2S) i wstrzymal server.handleClient().
  // W tych rzadkich zbieznych przypadkach przyjmij ulamkowy blad fazy sekundy RTC
  // (jednorazowy, nieskumulowany — to swiezy zapis z NTP).
  if (!playing && !ringing && !webActive &&
...
// Jednorazowa kalibracja sprzetowa dryftu krysztalu (rejestr Offset 0x02).
// Niezalezna od rejestrow czasu (0x04+) i markera (0x03) — writeRtc ich nie rusza,
// wiec wpis trwa przez kolejne synchronizacje NTP. Wolane raz przy kazdym starcie:
// rejestr Offset jest podtrzymywany bateryjnie, ale ponowny zapis gwarantuje
// poprawna wartosc takze po factory-reset / utracie zasilania RTC.
static bool rtcApplyOffsetCalibration() {
  if (!rtcBeginSession()) return false;
  return rtcWriteReg(RTC_OFFSET_REG, RTC_OFFSET_VALUE);
}
...
static bool rtcConfigureWakeInterrupt(bool alarmEnabled) {
  ...
  return rtcWriteReg(0x01, control2);                // AF=TF=MI=HMI=0, CLKOUT off
}
...
static bool rtcVerifyWakeInterruptConfig(bool alarmEnabled) {
  ...
  uint8_t timerValue = timer[0];
  uint8_t timerMode = timer[1];
...
static bool rtcVerifyWakeAlarm(time_t epoch) {
  ...
  if (!rtcBeginSession()) return false;
  uint8_t alarmRegs[5];
  if (!rtcReadRegs(0x0B, alarmRegs, 5)) return false;
...
//  Ustawienia (NVS)
// ============================================================================
static const char *NVS_NS = "zegar3";   // przestrzen nazw Preferences (alarmy, WiFi, drain, liczniki resetow)
...
// ============================================================================
//  Diagnostyka produkcyjna — przyczyna resetu i jej liczniki
// ============================================================================
...
// Wczytaj dozywotnie liczniki resetow z NVS i — jesli ten rozruch jest skutkiem
// brownoutu/panic/WDT — zwieksz odpowiedni licznik. Zdarzenia rzadkie, wiec
// zapis do flash jest bezpieczny. Cold boot / deep-sleep nie zapisuja nic.
static void recordResetReason(esp_reset_reason_t r) {
  ...
  if (r == ESP_RST_BROWNOUT) {
    prefs.putUInt("rstBrown", ++g_brownoutResets);
  } else if (r == ESP_RST_TASK_WDT || r == ESP_RST_INT_WDT || r == ESP_RST_WDT) {
    prefs.putUInt("rstWdt", ++g_wdtResets);
  } else if (r == ESP_RST_PANIC) {
    prefs.putUInt("rstPanic", ++g_panicResets);
  }
  prefs.end();
}
...
static bool wifiBeginSmart() {
  ...
  if (g_wifiHint) {
    WiFi.begin(wifiSsid, wifiPass, g_wifiChannel, g_wifiBssid);
    return true;
  }
#endif
...
static bool ensureWifi(uint32_t timeoutMs) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    while (WiFi.status() != WL_CONNECTED && millis() - start < timeoutMs) {
      if (usedHint && !retriedFullScan &&
          millis() - start >= WIFI_HINT_FALLBACK_MS) {
        retriedFullScan = true;
        wifiRetryFullScan();
      }
      delay(100);
      yield();
      watchdogFeed();   // laczenie WiFi blokuje do timeoutMs (~15 s) — nie pozwol na falszywy reset
    }
  }
...
static bool serviceNtp() {
  if (ntpSyncState == NTP_SYNC_WIFI_CONNECTING) {
    if (WiFi.status() == WL_CONNECTED) {
      wifiCacheHint();
      beginNtpRequest();
    } else if (ntpWifiHintActive &&
               millis() - ntpStateStartedMs >= WIFI_HINT_FALLBACK_MS) {
      ntpWifiHintActive = false;
  ...
  if (g_ntpGotPacket.load(std::memory_order_acquire)) {
    ...
    // Dryft RTC: porownaj STARY czas RTC (jeszcze nie nadpisany) z czasem NTP.
    // Na pierwszej synchronizacji (RTC pusty) readRtcEpoch zawodzi -> brak wpisu.
    {
      time_t oldRtc;
...
// Zapis zwiezlego podsumowania zuzycia do NVS najwyzej raz na dobe (nadpisywany,
...
static void maybeWriteDailySummary(time_t now) {
  ...
  g_lastSummaryDay = day;                      // oznacz dzien nim sprobujesz zapisu (R1): martwy NVS nie ma byc ponawiany co petle
  if (!prefs.begin(NVS_NS, false)) return;
...
#if ENABLE_DIAGNOSTICS
    if (g_coldBootEpoch == 0) g_coldBootEpoch = (int64_t)nowT;   // kotwica czasu pracy
#endif
  }
...
// Tryb oszczedny: pomiar wiarygodny i SoC <= prog. Wstrzymuje energochlonne WiFi/NTP.
static bool batteryLow() {
  return batteryHasReading && batteryPercent <= BATTERY_LOW_PERCENT;
}

// Stan krytyczny: ogniwo blisko bezpiecznego minimum. updateBattery() bramkuje
// pomiar pod obciazeniem, wiec batteryVoltage to napiecie spoczynkowe (OCV).
...
static int nextAlarmIndex(uint8_t *h, uint8_t *mi, uint8_t *s, char *label, size_t labelSize) {
  ...
  // czasu kolejnosc wg godzin sciennych klamie (02:45 dzwoni PO 03:15), a ekran
...
// Sygnatura wszystkiego, co RYSUJE strona zegara (drawClockPage): HH:MM, %aku,
// stan/godzina najblizszego budzika, WWW on/off. Pola niepokazywane tutaj
// (ntpStatus, statusText) nie wchodza do sygnatury -> brak pelnoklatkowego
// sendBuffer() identycznej zawartosci na panelu refleksyjnym (zero mrugniec).
static uint32_t clockSig(const struct tm &t, int day) {
  ...
  mix(next >= 0 ? (uint32_t)(nh * 60 + nm + 1) : 0u);  // +1: brak budzika != 00:00
  return h;
}
...
static void drawPreviewPage() {
  ...
  gfx->sendBuffer();
  lastClockSig = 0xFFFFFFFFu;   // powrot do zegara musi wymusic repaint
}
...
static void drawNightPage() {
  ...
  gfx->sendBuffer();
  displayDirty = false;
  lastClockSig = 0xFFFFFFFFu;   // po wybudzeniu zegar zawsze repaint
}
...
static void updateDisplay() {
  ...
  if (!getLocalTm(&t)) {
    if (displayDirty) {
      ...
      gfx->sendBuffer();
      displayDirty = false;
      lastClockSig = 0xFFFFFFFFu;   // gdy czas wroci, wymus repaint zegara
    }
    return;
  }
  ...
  // Repaint TYLKO gdy zmienia sie widoczna tresc zegara. displayDirty ustawiane
  // przez zmiany statusu NTP/MP3 (niepokazywane tutaj) jest jedynie konsumowane,
  // wiec synchronizacja NTP i sesja WWW nie wywoluja repaintu identycznej klatki.
  if (sig != lastClockSig) {
    drawClockPage(t);
...
static void handleTest() {
  // Nie przerywaj dzwoniacego budzika testem z panelu: startSound() nie kasuje
  // stanu ringing (userStopped=false), wiec serviceRinging() wznowilby budzik po
  // RING_RETRY_MS i test bilby sie z budzikiem o kodek. Pomin test podczas dzwonienia.
  if (!ringing) {
...
static void handleMp3UploadStream() {
  webUntilMs = millis() + WEB_WINDOW_MS;
  watchdogFeed();   // callback wolany per fragment: upload duzego MP3 blokuje loop() minutami
...
#if ENABLE_DIAGNOSTICS
// Strona /info — diagnostyka produkcyjna (tylko odczyt, bez danych wrazliwych).
static void handleInfo() {
  ...
  // Czas pracy od zimnego startu (RTC RAM przezywa noce; znika po wymianie ogniwa).
  {
    time_t now = time(nullptr);
    if (isSupportedEpoch(now) && g_coldBootEpoch > 0) {
      long s = (long)(now - (time_t)g_coldBootEpoch);
      if (s < 0) s = 0;
      char buf[48];
      snprintf(buf, sizeof(buf), "%ld dni %02ld:%02ld:%02ld",
               s / 86400, (s % 86400) / 3600, (s % 3600) / 60, s % 60);
      row("Czas pracy (od zimnego startu)", buf);
    } else {
      row("Czas pracy (od zimnego startu)", "ustala sie");
    }
  }
  ...
  if (g_lastNtpSyncEpoch > 0 && g_prevNtpSyncEpoch > 0) {
    long winH = (long)((g_lastNtpSyncEpoch - g_prevNtpSyncEpoch) / 3600);
    row("Dryft RTC (ostatni sync)", String(g_lastRtcDriftSec) + " s / ~" + String(winH) + " h");
  } else {
    row("Dryft RTC (ostatni sync)", "brak danych (min. 2 synchronizacje)");
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    row("WiFi", WiFi.SSID() + "  RSSI " + String(WiFi.RSSI()) + " dBm");
  ...
  html += F("</table><p><a href='/'><button>Powrot</button></a></p></section>");
  html += F("</main></body></html>");
  server.send(200, "text/html", html);
}
...
static void stopWebWindow() {
  server.stop();
  webActive = false;
  // Aktywna proba NTP trwa najwyzej kilkadziesiat sekund (timeout pakietu 50 s,
  // NTP_PACKET_TIMEOUT_MS). Pozwol jej zakonczyc walidacje i zapis RTC;
  // finishNtpAttempt() sam wtedy wylaczy WiFi.
...
static void checkNtpSchedule() {
#if ENABLE_LOW_BATTERY_PROTECT
  // Tryb oszczedny: gdy mamy poprawny czas, NIE budz WiFi do auto-synchronizacji
  // (najwiekszy pobor pradu). Zegar chodzi z RTC; reczny sync z panelu WWW dziala.
  // Gdy czasu brak (pusty RTC), pozwol sie synchronizowac mimo niskiej baterii.
  ...
  int day = localDayKey(t);
  // Preferowane okno 10:00-10:05; jesli pominiete (np. dlugi upload MP3 blokuje
  // petle przez kilka minut), dogon synchronizacje o dowolnej porze az do poczatku
  // nocy. ntpStartedDay gwarantuje dokladnie jeden sync na dobe.
...
#if ENABLE_LOW_BATTERY_PROTECT
// Ochronny deep-sleep przy krytycznym stanie ogniwa: chroni Li-ion przed
// glebokim rozladowaniem i sprzetowym brownoutem (ryzyko uszkodzenia FLASH przy
// zapisie). Budzi sie cyklicznie (timer), by sprawdzic ladowanie; KEY pozwala
// wybudzic recznie. Pomijane przy dzwoniacym budziku / audio / sesji WWW /
// podgladzie / aktywnym NTP/zapisie RTC — nie przerywamy interakcji ani alarmu
// (zostaje wtedy ostrzezenie na ekranie, a sen nastapi po powrocie do spoczynku).
// Deep-sleep resetuje chip -> po wybudzeniu setup() ponownie zmierzy ogniwo i
// albo wznowi prace (gdy podlaczono ladowarke), albo usnie dalej.
static void enterCriticalBatterySleepIfNeeded() {
  ...
  esp_sleep_enable_ext1_wakeup((1ULL << KEY_PIN), ESP_EXT1_WAKEUP_ANY_LOW);  // reczne wybudzenie KEY
...
static void enterNightSleepIfNeeded(bool force) {
  ...
  if (!rtcAlarmOk) {
    // Budzi tylko timer ESP na niedokladnym oscylatorze RC. Gdy najblizszym
    // zdarzeniem jest realny budzik (wakeAt skrocone do alarmu powyzej), tnij
    // odcinki krocej: dryft RC ostatniego odcinka musi zostac duzo ponizej okna
    // wylapania NTP_ALARM_CATCHUP_SEC, inaczej spoznione wybudzenie gubi budzik.
...
#endif
  watchdogFeed();              // tuz przed snem: licznik TWDT rusza od zera
  esp_light_sleep_start();
  watchdogFeed();              // tuz po wybudzeniu: pojedynczy sen trwa do 60 s
...
void setup() {
  ...
  // start) oraz o akcji po wybudzeniu (KEY -> panel WWW, BOOT -> podglad,
  // RTC/timer -> wybudzenie planowe). Ekran inicjalizujemy tak samo przy KAZDYM
  // wybudzeniu (patrz setupDisplay).
  ...
  Serial.begin(115200);
  if (coldBoot) delay(300);            // delay tylko na zimnym starcie (USB-CDC); wybudzenia z deep-sleep go pomijaja
  setCpuFrequencyMhz(DAY_CPU_MHZ);
  watchdogSetup();                     // nadzoruj loopTask juz od setup() (timeout WDT_TIMEOUT_S)
  releaseNightHolds();
  pinMode(PA_ENABLE_PIN, OUTPUT);
  digitalWrite(PA_ENABLE_PIN, LOW);
  setupDisplay(coldBoot);              // pelny re-init (reset RST + begin) przy KAZDYM wybudzeniu; brak mrugania w dzien zapewnia light-sleep (setup() sie nie powtarza)
  ...
  if (coldBoot) {                       // tylko cold boot - RTC RAM przezywa deep-sleep
    ...
    g_lastSummaryDay = -1;
    g_wifiHint = false;
    g_bootCount = 0;                    // diagnostyka: zerowane tylko na zimnym starcie
    g_coldBootEpoch = 0;
    g_lastRtcDriftSec = 0;
    g_lastNtpSyncEpoch = 0;
    g_prevNtpSyncEpoch = 0;
  }
  g_bootCount++;                        // cold boot -> 1; kazde wybudzenie z deep-sleep -> +1
  ...
  loadSettings();
#if ENABLE_DIAGNOSTICS
  recordResetReason(esp_reset_reason());   // policz brownout/WDT/panic (NVS, zdarzenia rzadkie)
#endif
  Serial.printf("\n=== %s v%s  (build %s %s) ===\n",
  ...
  rtcApplyOffsetCalibration();          // kalibracja dryftu krysztalu (rejestr Offset 0x02)
...
#if ENABLE_DIAGNOSTICS
  { time_t t0 = time(nullptr);          // zakotwicz czas pracy, gdy czas juz znany
  ...
  if (coldBoot) ensureFsMounted();   // FS potrzebny tylko do MP3; cieplo montuj leniwie (startSound/web) -> krotsze wybudzenie
  checkAlarms(scheduledWake ? NTP_ALARM_CATCHUP_SEC : ALARM_LATE_WINDOW_SEC);
  if (!userWake && !ringing) enterNightSleepIfNeeded(true);
#if ENABLE_LOW_BATTERY_PROTECT
  // Ochrona ogniwa juz na starcie: po planowym wybudzeniu z krytycznym poziomem
  // od razu wracaj do ochronnego snu. Wybudzenie uzytkownika (KEY/BOOT) pomijamy,
  // by mogl zobaczyc stan i podlaczyc ladowarke.
...
#if ENABLE_LOW_BATTERY_PROTECT
             !batteryLow() &&          // tryb oszczedny: pomin poranny re-sync NTP
#endif
...
void loop() {
  watchdogFeed();                  // nakarm watchdoga na poczatku kazdej iteracji
  applyCpuPolicy();
  handleKey();
  handleBoot();
  ...
  updateBattery(false);
#if ENABLE_LOW_BATTERY_PROTECT
  enterCriticalBatterySleepIfNeeded();   // ochronny sen, gdy ogniwo krytyczne i petla bezczynna
#endif

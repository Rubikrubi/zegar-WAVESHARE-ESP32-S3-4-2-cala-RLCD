// ============================================================================
//  Zegar NTP RLCD v10.0 — energooszczednosc: panel ST7305 w LPM (fix A audytu)
//
//  Zmiany v10.0 (wzgledem v9.0):
//   [ENERGIA] DisplayUi::setup(): po panel_.begin() wysylany jest cmd 0x39 (LPM)
//     -> odswiezanie panelu 1 Hz zamiast 32 Hz (HPM). Panel byl jedynym ukladem
//     nigdy nie usypianym przez firmware (HPM 32 Hz 24/7); LPM zdejmuje ~0,25-0,45 mA
//     sredniego poboru. Zegar odswieza sie raz na minute, wiec 1 Hz wystarcza.
//     Ryzyko: mozliwy ghosting przy 1 Hz — do weryfikacji na sprzecie.
//   Reszta zachowania bez zmian wzgledem v9.0.
//
//  Zegar NTP RLCD v9.0 — wzorcowy refaktor architektury (funkcjonalnie = v7.1)
//
//  ZASADA NACZELNA: zachowanie identyczne z v7.1 co do bajta (NVS, HTML, rejestry,
...
//      pracuja na tych samych danych liczonych RAZ (v7.1 liczyl najblizszy budzik
...
//   [POPRAWKI BLEDOW ODZIEDZICZONYCH PO v7.1 — jedyne swiadome zmiany zachowania]
...
//   Pozostale inwarianty czasu/energii/RTC/NTP/sleep przeniesione 1:1 z v7.1
...
//  odzysk zawieszonej szyny I2C, NTP fail-fast po zerwaniu WiFi; v7.1 porzadki.
//
...
// ES7210 (4-kanalowy ADC mikrofonow, I2C 0x40 na wspolnej szynie) — firmware go
// nie uzywa, ale siedzi na stale zasilonej szynie A3V3 z MICBIAS dla dwoch
// mikrofonow MEMS, a stan po power-on jest nieudokumentowany. Jawny power-down
// (~10 uA wg datasheetu); sekwencja 1:1 z es7210_stop() referencyjnego drivera
// Waveshare (esp_codec_dev). Wolane raz w setup(), gdy Wire juz dziala.
static void es7210PowerDown() {
  ...
  wr(0x00, 0xFF); wr(0x00, 0x41);                                 // wake-up reset: czesc
                                                                  //  rewizji ignoruje zapisy bez tego
  wr(0x47, 0xFF); wr(0x48, 0xFF); wr(0x49, 0xFF); wr(0x4A, 0xFF); // MIC1..4 off
  wr(0x4B, 0xFF); wr(0x4C, 0xFF);                                 // zasilanie + bias MIC12/34 off
  wr(0x40, 0xC0);                                                 // sekcja analogowa off
  wr(0x01, 0x7F);                                                 // zegary off
...
public:
  ...
  // Pelne uspienie: sekwencja suspend z referencyjnego es8311.c (esp-adf) +
  // user guide 9.1/9.2. Sam reset (0x00=0x1F) NIE cofa rejestrow analogowych
  // zapisanych w begin() — bez ponizszych zapisow bias analogowy (~6 mA na
  // stale zasilonej szynie A3V3) grzeje sie na zawsze od pierwszego
  // odtworzenia. Kolejnosc: mute -> analog off -> zegary off -> reset.
  bool powerDown() {
    bool ok = true;
    ok &= write(es8311::REG_DAC_VOLUME, 0x00);
    ok &= write(0x17, 0x00);              // glosnosc ADC = 0
    ok &= write(0x0E, 0xFF);              // PDN_PGA/PDN_MOD/RST_MOD: tor nagrywania off
    ok &= write(0x12, 0x02);              // PDN_DAC=1
    ok &= write(0x14, 0x00);              // mux wejsciowy off, PGA 0
    ok &= write(0x0D, 0xFA);              // bias/VREF analogowe off (0xFA jak esp-adf:
                                          //  VMID zostaje -> wznowienie bez strzalu)
    ok &= write(0x15, 0x00);
    ok &= write(0x37, 0x08);
    ok &= write(0x45, 0x01);
    ok &= write(es8311::REG_CLK_MGR1, 0x00);  // wszystkie galezie zegarow zamkniete
    ok &= write(es8311::REG_RESET, 0x1F);     // CSM off + reset — NA KONCU
    return ok;
  }
...
void NetworkTimeService::checkSchedule() {
  ...
  // straznika okna wieczornego). W v7.1 to okno zylo tylko w setup(): urzadzenie
  ...
  // natychmiast, wiec timing wzgledem v7.1 bez zmian.
...
// -> panel trzyma obraz -> brak mrugania.
//
// v10: po begin() przelaczamy panel w LPM (Low Power Mode, cmd 0x39) — odswiezanie
// wewnetrzne 1 Hz zamiast 32 Hz w HPM (cmd 0x38, ktory U8g2 wysyla w init). Zegar
// odswieza sie raz na minute, wiec 1 Hz w zupelnosci wystarcza, a panel przestaje
// taktowac swoja matryce z 32 Hz 24/7 (oszczednosc ~0,25-0,45 mA sredniego poboru).
// Jedno wyslanie wystarcza: deep-sleep robi pelny re-init w setup(), a light-sleep
// nie resetuje panelu. NIE uzywac setPowerSave(1) — to cmd 0x28, ktore GASI obraz.
// UWAGA: przy 1 Hz mozliwy ghosting — zweryfikowac na sprzecie; w razie problemow
// wrocic do 0x38 (HPM) lub podbic odswiezanie.
void DisplayUi::setup() {
  ...
  panel_.begin();
  panel_.sendF("c", 0x39);   // LPM: odswiezanie 1 Hz zamiast 32 Hz (HPM)
  delay(100);                // datasheet 7.11: odczekac ~1 ramke po przelaczeniu
...
void ButtonInput::pollKey() {
  switch (keyButton_.poll()) {
    ...
    case ButtonEvent::Released:
      ...
      // od zimnego startu (fix v8.0 — v7.1 odpalal wtedy panel WWW).
...
void App::setup() {
  ...
  amplifierOff();
  // Parkowanie linii I2S: od resetu do pierwszego audio plywaja jako wejscia,
  // wiszac na wejsciach zegarowych stale zasilonych ES8311/ES7210 — plywajace
  // wejscie CMOS w okolicy progu = prad skrosny bufora odbiornika. Stan NISKI
  // to poprawny poziom logiczny. Pierwsze odtwarzanie przejmuje piny z
  // powrotem: setPinout() -> i2s_channel_reconfig_std_gpio (audioI2S 3.4.5).
  for (gpio_num_t p : { pins::I2S_MCLK, pins::I2S_BCLK, pins::I2S_WS, pins::I2S_DOUT }) {
    pinMode(p, OUTPUT);
    digitalWrite(p, LOW);
  }
  gpio_reset_pin(GPIO_NUM_10);    // ASDOUT z ES7210 (nieuzywany): pull-up, bufor odciety
  display_.setup();
  ...
  es7210PowerDown();                     // nieuzywany ADC mikrofonow tez (Wire juz dziala)
